...
| `helper(0,2)` | 0 | 2 | b | c | (0,2) = `first_match = false` | `b` no coincide con `a` y el siguiente no es `*`→ se retorna `memo[key=(0,0)] = false`.|
...
| `helper(1,2)` | 1 | 2 | b | c | (1,2) = `first_match = false` | `b` no coincide con `a` y el siguiente no es `*`→ se retorna `memo[key=(1,2)] = false`. |
...
Obtenemos una complejidad **O(m × n)**, siendo `m = len(s)` y `n = len(p)`. Los posibles estados del subproblema son todos los pares `(index_s, index_p)`, con **index_s ∈ [0, m]** e **index_p ∈ [0, n]**, lo que da a lo sumo **(m+1) × (n+1)** subproblemas.\
Gracias a la memoización, cada uno se calcula exactamente una vez, y el trabajo por subproblema es O(1).

### Espacial

El diccionario `memo` almacena como máximo **(m + 1) × (n + 1)** entradas. A esto se suma el espacio de la pila de recursión, que en el peor caso tiene profundidad **O(m + n)** cuando no hay `*` y se avanza un índice por llamada. 
...
- La recursión implica overhead de llamadas a función y uso de pila. En lenguajes sin optimización de tail-call (como Python), instancias muy grandes pueden provocar `RecursionError`.
