...
Como `nums` siempre está ordenado, podemos usar `bisect` para encontrar la posición de inserción en `O(log n)`, pero el conteo de menores y mayores sigue costando `O(n)` por elemento —ahí está el cuello de botella.
...
Con `n = 10⁵`, esto implica del orden de `10¹⁰` operaciones en el peor caso → excede el límite de tiempo de LeetCode (TLE).
...
- No escala con las restricciones del problema (`n ≤ 10⁵`): produce TLE en LeetCode.
