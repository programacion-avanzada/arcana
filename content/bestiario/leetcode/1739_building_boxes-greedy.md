...
- **Comparación con la Solución 2 (forma cerrada).** Las dos usan **la misma estrategia voraz**
  ...
  ($O(1)$, pero con punto flotante que hay que blindar). Regla práctica: **iterativo para
...
resuelve iterando, sino **dando vuelta una fórmula**: se despeja el valor exacto con una sola
...
### Paso 1: dar vuelta la fórmula de la pirámide (la cúbica)
...
![Paso 1 Rpresentacion Cubica](paso1_recta_cubica.svg)
...
def minimumBoxes(n):
    ...
    # Paso 1: mayor r con Tet(r) <= n, dando vuelta la cúbica en O(1)
    ...
    # Paso 2: menor p con p(p+1)/2 >= sobrantes, dando vuelta la cuadrática
...
- **Limitaciones:** depende de **punto flotante** (raíces); para $n$ muy grande hay que blindar
