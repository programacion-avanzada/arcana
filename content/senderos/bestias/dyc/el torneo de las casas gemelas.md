...
3. Escribe la recurrencia de Karatsuba: $T(n) = 3 \cdot T(n/2) + O(n)$. Aplica el Teorema Maestro y obtén $O(n^{\log_2 3}) \approx O(n^{1.585})$.
...
| -------------------- | ------------ | --------------------------------- |
| Multiplicación naïve | $O(n^2)$      | Fuerza bruta                      |
